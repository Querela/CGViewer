...
Scene::Scene(QWidget *parent)
{
	...
	shadersAvaiable = initShaders("Shaders/PhongShader_vert.c", "Shaders/PhongShader_frag.c");
...
void Scene::setUseShaders(bool u)
{
	...
	if (useShaders) //reinit for faster debugging
		initShaders("Shaders/PhongShader_vert.c", "Shaders/PhongShader_frag.c");
	updateGL();
}
...
{
	//create File
    ofstream file(path.toUtf8().data());
	...
	for (size_t i=0;i<nMat;++i)
	{
		...
		if (isTexture > 0) //load texture
		{
			...
			if (texture.isNull()) //try the "textures" sub directory
			{
                ...
                //cout<<".....failed: try Texture: "<<filepath.toUtf8()<<endl;
...
void Scene::paintGL()
{
	...
	if (useShaders)
	{
		...
		if (size == 0) //the standard light (camera) is on
			size = 1;
		glUniform1i(glGetUniformLocation(phongShaderId,"numLights"), size);
		glUniform1i(glGetUniformLocation(phongShaderId,"basemap"), 0);
		glUniform1i(glGetUniformLocation(phongShaderId,"normalmap"), 1);


		//cout<<"basemap on: "<<glGetUniformLocation(phongShaderId,"basemap")<<endl;
		//cout<<"normalmap on: "<<glGetUniformLocation(phongShaderId,"normalmap")<<endl;
		//cout<<"numLights on: "<<glGetUniformLocation(phongShaderId,"numLights")<<endl;
